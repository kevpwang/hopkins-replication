...
exp1 <- read.dta("data/accentsexp1.dta") %>% 
  clean_names()
exp2 <- read.dta("data/accentsexp2.dta") %>% 
  clean_names()
...
exp1_new <- exp1 %>% 
  ...
  mutate(republican = case_when(grepl("Not Strong Rep", party7) ~ 2,
                                grepl("Not Strong Dem", party7) ~ 6,
                                grepl("Leans Rep", party7) ~ 3,
                                grepl("Leans Dem", party7) ~ 5,
...
exp2_new <- exp2 %>% 
  ...
  filter(conservative != -1) %>% 
  mutate(republican = case_when(grepl("Not Strong Rep", partyid7) ~ 2,
